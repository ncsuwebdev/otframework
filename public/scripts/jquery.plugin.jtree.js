	...
	$.fn.jTree = function(options) {
		...
		$(this).find("li").mousedown(function(e){
			if ($("#jTreeHelper").is(":not(:animated)") && e.button !=2) {
				...
				// Allow for clickable links and images
				if (!$(e.target).hasClass('moveHandle')) { 
					if (e.target.tagName.toLowerCase() == 'a' || e.target.tagName.toLowerCase() == 'span' || new String(e.target).match(/(f|ht)tp(s{0,1}):\/\//ig)) {
				 		return false;
					}
				}
