...
<div id="userIndexIndex">
    {if $acl.add}
    <span class="addButton"><a href="{$sitePrefix}/account/index/add/">Add New Account</a></span>
    <br /><br />
    {/if}
    {editable id="user"}
    ...
    {foreach from=$users item=u name=users}
        ...
        <tr class="{cycle values="row1,row2"}">
            ...
            {if $acl.edit}
            <td align="center">
                <a href="{$sitePrefix}/account/index/edit/?userId={$u.userId}"><img src="{$sitePrefix}/public/ot/images/edit.png" alt="Edit {$u.userId}" /></a>
            </td>
            {/if}
            {if $acl.delete}
            <td align="center">
                {if $u.userId != $loggedInUserId}
                <a href="{$sitePrefix}/account/index/delete/?userId={$u.userId}"><img src="{$sitePrefix}/public/ot/images/delete.png" alt="Delete {$u.userId}" /></a>
                {else}
                <img src="{$sitePrefix}/public/ot/images/deleteDisabled.png" alt="Delete Disabled for {$u.userId}" />
                {/if}
            </td>
            {/if}
        </tr>
