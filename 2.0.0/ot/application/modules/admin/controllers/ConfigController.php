...
{   
    ...
    {       
        ...
        $this->_helper->pageTitle('Configuration Admin');
    }
    ...
    {       
        ...
        $overrideFile = $config->app->overridePath . '/config/config.xml';
        
        if (!file_exists($overrideFile)) {
            throw new Ot_Exception_Data("Config override file not found.");
        }
        
        if (!is_writable($overrideFile)) {
            throw new Ot_Exception_Data('Config override file (' . $overrideFile . ') is not writable, therefore it cannot be edited');
        }
        ...
        $reset = $form->createElement('checkbox', 'resetToDefault', array('label' => 'Reset to Default: '));
        ...
        if ($this->_request->isPost()) {
        	if ($form->isValid($_POST)) {
	            ...
	            $this->_helper->flashMessenger->addMessage('The value for ' . $get->key . ' has been updated successfully!');
	            
	            $this->_helper->redirector->gotoUrl('/admin/config/');
        	} else {
        		$messages[] = 'There was an error with your form';
        	}
        }
        ...
        $this->_helper->pageTitle('Edit Configuration Option');
    }
}