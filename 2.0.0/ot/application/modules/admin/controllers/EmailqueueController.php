...
class Admin_EmailqueueController extends Zend_Controller_Action
{
    ...
    {
        $this->_helper->pageTitle('Email Queue');  
        ...
        if ($this->_request->isXmlHttpRequest()) {
        	...
        	foreach ($emails as $e) {
        		$row = array(
        			'id'   => $e['emailQueueId'],
        			'cell' => array(
        				'to' => implode(', ', $e['zendMailObject']->getRecipients()),
                		'subject' => $e['zendMailObject']->getSubject(),        		
        				ucwords($e['status']),
        				strftime($config->user->dateTimeFormat->val, $e['queueDt']), 
        				strftime($config->user->dateTimeFormat->val, $e['sentDt']),
        				$e['attributeName'],
        				$e['attributeId'],       				
        			),
        		);
    ...
    public function detailsAction()
    {
        ...
        $this->_helper->pageTitle('Queued Email Details');
    }
}