 ...
 * @copyright  Copyright (c) 2007 NC State University Office of Information Technology
 */
class Admin_CustomController extends Zend_Controller_Action
{
    /**
    ...
    public function indexAction()
    {
        ...
        $this->_helper->pageTitle('Custom Fields');
    ...
    public function detailsAction()
    {
        ...
        $this->_helper->pageTitle('Attributes for ' . $get->objectId);
    ...
    public function saveAttributeOrderAction()
    {
        ...
        if ($this->_request->isPost()) {
            ...
            if (!isset($post->objectId)) {
                $ret = array('rc' => 0, 'msg' => 'Object ID was not set, please try again');
                echo Zend_Json_Encoder::encode($ret);
                return;
            }
            
            if (!isset($post->attributeIds)) {
                $ret = array('rc' => 0, 'msg' => 'Attribute IDs were not set, please try again');
                echo Zend_Json_Encoder::encode($ret);
                return;
            }
            ...
            try {
                ...
                $ret = array('rc' => 1, 'msg' => 'New order was saved successfully');
                echo Zend_Json_Encoder::encode($ret);
                return;
            } catch (Exception $e) {
            	$ret = array('rc' => 0, 'msg' => 'There was an error saving the order. ' . $e->getMessage() . ' Please try again.');
                echo Zend_Json_Encoder::encode($ret);
                return;
            }
    ...
    public function attributeDetailsAction()
    {
    	...
    	$this->_helper->pageTitle('Attribute Details');
    }
    ...
    public function addAction()
    {
        ...
        if ($this->_request->isPost()) {
            ...
            $this->_helper->flashMessenger->addMessage('Attribute ' . $data['label'] . ' successfully added to ' . $data['objectId']);

            $this->_helper->redirector->gotoUrl('/admin/custom/details/?objectId=' . $data['objectId']);
        }

        $this->view->types = $custom->getTypes();
	    $this->_helper->pageTitle('Add Custom Attribute to ' . $get->objectId);
	    $this->view->objectId = $get->objectId;
    ...
    public function editAction()
    {
        ...
        if ($this->_request->isPost()) {
            ...
            $this->_helper->flashMessenger->addMessage($data['label'] . ' successfully saved.');

            $this->_helper->redirector->gotoUrl('/admin/custom/details/?objectId=' . $attribute['objectId']);
        }

        $this->_helper->pageTitle('Edit Custom Attribute for ' . $attribute['objectId']);
        $this->view->objectId          = $attribute['objectId'];
        $this->view->objectDescription = $config->app->customFieldObjects->{$attribute['objectId']};
        $this->view->attribute         = $attribute;
        $this->view->types             = $custom->getTypes();
    ...
    public function deleteAction()
    {
        ...
        if ($this->_request->isPost() && $form->isValid($_POST)) {
            ...
            $this->_helper->flashMessenger->addMessage($attribute['label'] . ' successfully delete.');
        ...
        $this->view->objectId          = $attribute['objectId'];
        $this->view->objectDescription = $config->app->customFieldObjects->{$attribute['objectId']};
	    $this->_helper->pageTitle('Delete Attribute from ' . $attribute['objectId']);
    }
}