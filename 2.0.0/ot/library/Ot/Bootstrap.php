...
class Ot_Bootstrap
{
	...
	/**
	 * Boolean for whether or not to throw exceptions
	 * 
	 * @var bool
	 */
	...
	public function dispatch($dbConfig)
	{
        ...
        try {
            ...
            $req = new Zend_Session_Namespace($url . '_request');
            $req->unsetAll();
    ...
    /**
     * Sets the front controller to throw exceptions
     *
     */
    ...
    public function setupView()
    {
        ...
        if (!is_dir('public/themes/' . $theme)) {
            if (!is_dir('public/ot/themes/' . $theme)) {
                $theme = 'default';
                $themePath = 'public/ot/themes/default';
            } else {
                $themePath = 'public/ot/themes/' . $theme;
            }
        } else {
            $themePath = 'public/themes/' . $theme;
        }
     
        // Configure Zend_Layout
        $layout = Zend_Layout::startMvc($themePath . '/views/layouts/');
        
        $view = $layout->getView();
        
        $view->config = $config;
        $view->applicationThemePath = $themePath;
    ...
    public function setupFrontController()
    {
        ...
        $front->setBaseUrl($this->_baseUrl)
              ...
              ->setRouter($router)
              ->throwExceptions($this->_throwExceptions);
        ...
        $front->registerPlugin(new Zend_Controller_Plugin_ErrorHandler($eHandlerConfig))
              ...
              ->registerPlugin(new Ot_FrontController_Plugin_DebugMode())
