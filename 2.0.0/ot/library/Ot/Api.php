...
class Ot_Api
{
	 ...
	 *
	 * @return key => value array of frameworkName => Version
	 */
	 ...
	 * 
	 */
	public function updateMyAccount($firstName, $lastName, $emailAddress, $timezone)
	{
	    if (!Zend_Auth::getInstance()->hasIdentity()) {
	        throw new Ot_Exception_Access('msg-error-apiAccessDenied');
        }
	 ...
	 * @param string $timezone A valid timezone string
	 */
	public function updateAccount($accountId, $firstName, $lastName, $emailAddress, $timezone)
	{	    
	    if (!in_array($timezone, Ot_Timezone::getTimezoneList())) {
	        throw new Ot_Exception_Data('msg-error-invalidTimezone');
	    }
	 ...
	 */
	public function setCronJobStatus($name, $status)
	{
	 ...
	 */
	public function getBugReports($status = null)
	{
        $otBug = new Ot_Bug();
