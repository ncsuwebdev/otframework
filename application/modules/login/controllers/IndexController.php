...
class Login_IndexController extends Internal_Controller_Action 
{
        ...
        if ((isset($authRealm->realm) && $authRealm->autoLogin) || ($this->_request->isPost() && $validForm)) {
            ...
            if ($result->isValid()) {
	            ...
	            try {
	            	...
	            	if ($config->loginOptions->generateAuthzInstanceOnFirstLogin == 1) {
	            	    $authz->addUser($userId, (string)$config->loginOptions->defaultRoleOnAuthzInstanceCreation);
	            		$user = $authz->getUser($userId);
	            		$role = $user['role'];
	            	} else {
	            ...
	            if ($config->loginOptions->generateUserProfileOnFirstLogin == 1) {
	            	$profile = new Profile();
	            	$profile->insert(array('userId' => $userId));
	            }
    ...
    public function logoutAction()
    {
        ...
        $this->_logger->setEventItem('attributeName', 'userId');
        $this->_logger->setEventItem('attributeId', Zend_Auth::getInstance()->getIdentity());
    ...
    {    	
        ...
        $form->addElement($realmStatic)
             ...
             ->addElement($lastName)
             ->addElement($email)
             ->addDisplayGroup(array('realmStatic', 'username', 'password', 'passwordConf', 'firstName', 'lastName', 'emailAddress'), 'fields')
